...
def create_query_params_class(
    name: str, allowed_associations: List[str], allowed_order_by: List[str]
):
    class QueryParams(BaseModel):
        ...
        def check_association(cls, v):
            ...
            if v not in allowed_associations:
                raise ValueError(
                    f"Invalid association '{v}'. Allowed: {allowed_associations}"
                )
            return v
        ...
        def check_order_by(cls, v):
            ...
            if v not in allowed_order_by:
                raise ValueError(
                    f"Invalid order_by '{v}'. Allowed: {allowed_order_by}"
                )
            return v
